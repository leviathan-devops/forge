...
#
# FORGE_ROOT / PROJECT_ROOT are always derived from this script's location
# (repo root = parent of scripts/), never OPENCODE_WORKSPACE.
#
...
# --- Configuration (repo-relative; closes F-BUNDLE-HARDCODED-ROOT) ---

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
# Repo root = parent of scripts/ (this is FORGE_ROOT for the monorepo tree)
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
FORGE_ROOT="$PROJECT_ROOT"
FORGE_DIR="$FORGE_ROOT/forge"
RESOURCES_DIR="$FORGE_ROOT/iOS/FORGE/Resources"
BUNDLE_FILE="$RESOURCES_DIR/forge-bundle.js"
BUILD_SCRIPT="$SCRIPT_DIR/build-forge-bundle.mjs"

# Prefer forge/node_modules for resolution (belt-and-suspenders with mjs createRequire)
export NODE_PATH="${FORGE_DIR}/node_modules${NODE_PATH:+:$NODE_PATH}"
...
echo -e "${CYAN}[FORGE]${NC} FORGE_ROOT=$FORGE_ROOT"
...
# --- Check for package.json and node_modules (esbuild lives under forge/) ---
...
# Install dependencies if needed so esbuild resolves from forge/node_modules
if [[ ! -d "$FORGE_DIR/node_modules" ]] || [[ ! -d "$FORGE_DIR/node_modules/esbuild" ]]; then
    echo -e "${YELLOW}[FORGE]${NC} Installing dependencies in $FORGE_DIR ..."
    cd "$FORGE_DIR"
    if [[ "$RUNTIME" == "bun" ]]; then
        bun install
    else
...
if [[ ! -d "$FORGE_DIR/node_modules/esbuild" ]]; then
    echo -e "${RED}[FORGE] ERROR:${NC} esbuild missing under $FORGE_DIR/node_modules after install"
    exit 1
fi

echo -e "${CYAN}[FORGE]${NC} esbuild: $FORGE_DIR/node_modules/esbuild"
...
# Always invoke from repo root; build script derives paths from its own location
if [[ "$RUNTIME" == "bun" ]]; then
    (cd "$FORGE_ROOT" && bun "$BUILD_SCRIPT")
else
    (cd "$FORGE_ROOT" && node "$BUILD_SCRIPT")
fi
...
if [[ -f "$FORGE_ROOT/iOS/FORGE.xcodeproj/project.pbxproj" ]]; then
    if grep -q "$BUNDLE_FILENAME" "$FORGE_ROOT/iOS/FORGE.xcodeproj/project.pbxproj"; then
