...
struct SettingsSheet: View {
    ...
    private var apiConfigurationSection: some View {
        Section {
            ...
            if apiProvider.requiresAPIKey {
                ...
                // Free-key guidance for the default opencode Zen provider.
                Link(destination: URL(string: "https://opencode.ai/auth")!) {
                    HStack(spacing: 6) {
                        Image(systemName: "key.fill")
                        Text("Get a free key at opencode.ai/auth")
                    }
                    .font(.forgeCaption)
                    .foregroundStyle(SwiftUI.Color.forgeAccent)
                }
                .padding(.top, 4)
                .accessibilityIdentifier("zenAuthLink")
            }
            ...
            // Endpoint base URL (empty = provider default; e.g. OpenCode Go)
            HStack {
                Text("Endpoint URL")
                    .foregroundStyle(SwiftUI.Color.forgePrimaryText)
                Spacer()
                TextField("https://…", text: $apiBaseUrl)
                    ...
                    .foregroundStyle(SwiftUI.Color.forgeSecondaryText)
                    .font(.forgeBodyMono)
                    .onChange(of: apiBaseUrl) { _, _ in hasUnsavedChanges = true }
            }
            .accessibilityIdentifier("apiBaseUrlField")
        } header: {
    ...
    private var aboutSection: some View {
        Section {
            ...
            Link(destination: URL(string: "https://github.com/leviathan-devops/forge")!) {
                HStack {
