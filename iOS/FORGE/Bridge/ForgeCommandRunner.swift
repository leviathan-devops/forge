...
final class ForgeCommandRunner {
    ...
    /// and jails the result under `projectRoot` (F-PATH-JAIL-ESCAPE residual
    /// scrub — CommandRunner must match ForgeBridge.resolveProjectPath).
    ///
    /// Absolute paths and `..` segments are accepted only when the standardized
    /// result remains under the project root; otherwise returns `nil`.
    private func resolvePath(_ arg: String, cwd: String) -> String? {
        let rootRaw = projectRoot.isEmpty ? cwd : projectRoot
        ...
        if cleaned.isEmpty {
            joined = root
        } else if (cleaned as NSString).isAbsolutePath {
            joined = (cleaned as NSString).standardizingPath
        } else {
            // Relative paths: resolve against cwd when cwd is inside the jail,
            // otherwise against project root.
            let cwdStd = (cwd as NSString).standardizingPath
            let rootPrefix = root.hasSuffix("/") ? root : root + "/"
            let base: String
            if cwdStd == root || cwdStd.hasPrefix(rootPrefix) {
                base = cwdStd
            } else {
                base = root
            }
            joined = ((base as NSString).appendingPathComponent(cleaned) as NSString)
                .standardizingPath
        }
    ...
    /// Fail-closed CommandResult when a path escapes the project jail.
    private func pathJailReject(_ cmd: String, _ arg: String) -> CommandResult {
        return CommandResult(
            stdout: "",
            stderr: "\(cmd): path escapes project root: \(arg)",
            exitCode: 1
        )
    }
    ...
    func cmdLs(_ args: String, cwd: String) -> CommandResult {
        ...
        guard let path = resolvePath(cleaned, cwd: cwd) else {
            return pathJailReject("ls", cleaned.isEmpty ? "." : cleaned)
        }
        do {
    ...
    func cmdCat(_ args: String, cwd: String) -> CommandResult {
        let cleaned = stripFlags(args)
        guard let path = resolvePath(cleaned, cwd: cwd) else {
            return pathJailReject("cat", cleaned.isEmpty ? "." : cleaned)
        }
        do {
    ...
    func cmdGrep(_ args: String, cwd: String) -> CommandResult {
        ...
        if nonFlags.count > 1 {
            guard let resolved = resolvePath(nonFlags[1], cwd: cwd) else {
                return pathJailReject("grep", nonFlags[1])
            }
            searchPath = resolved
        } else {
            // Default search root must still be inside the jail.
            guard let resolved = resolvePath("", cwd: cwd) else {
                return pathJailReject("grep", ".")
            }
            searchPath = resolved
        }
    ...
    func cmdFind(_ args: String, cwd: String) -> CommandResult {
        ...
        if !tokens.isEmpty && !tokens[0].hasPrefix("-") {
            guard let resolved = resolvePath(tokens[0], cwd: cwd) else {
                return pathJailReject("find", tokens[0])
            }
            basePath = resolved
            idx = 1
        } else {
            guard let resolved = resolvePath("", cwd: cwd) else {
                return pathJailReject("find", ".")
            }
            basePath = resolved
        }
    ...
    private func matchesGlob(_ name: String, pattern: String) -> Bool {
        return Self.globMatch(name: name, pattern: pattern)
    }
    ...
    func cmdMkdir(_ args: String, cwd: String) -> CommandResult {
        ...
        guard let path = resolvePath(cleaned, cwd: cwd) else {
            return pathJailReject("mkdir", cleaned)
        }
        do {
    ...
    func cmdRm(_ args: String, cwd: String) -> CommandResult {
        ...
        guard let path = resolvePath(cleaned, cwd: cwd) else {
            return pathJailReject("rm", cleaned)
        }
        do {
    ...
    func cmdCp(_ args: String, cwd: String) -> CommandResult {
        ...
        guard let src = resolvePath(tokens[0], cwd: cwd) else {
            return pathJailReject("cp", tokens[0])
        }
        guard let dst = resolvePath(tokens[1], cwd: cwd) else {
            return pathJailReject("cp", tokens[1])
        }
        do {
    ...
    func cmdMv(_ args: String, cwd: String) -> CommandResult {
        ...
        guard let src = resolvePath(tokens[0], cwd: cwd) else {
            return pathJailReject("mv", tokens[0])
        }
        guard let dst = resolvePath(tokens[1], cwd: cwd) else {
            return pathJailReject("mv", tokens[1])
        }
        do {
            try? FileManager.default.removeItem(atPath: dst)
    ...
    func cmdWc(_ args: String, cwd: String) -> CommandResult {
        ...
        guard let path = resolvePath(cleaned, cwd: cwd) else {
            return pathJailReject("wc", cleaned)
        }
    ...
    func cmdHead(_ args: String, cwd: String) -> CommandResult {
        ...
        guard let path = resolvePath(cleaned, cwd: cwd) else {
            return pathJailReject("head", cleaned)
        }
        guard let content = try? String(contentsOfFile: path, encoding: .utf8) else {
    ...
    func cmdTail(_ args: String, cwd: String) -> CommandResult {
        ...
        let cleaned = stripFlags(fileArg)
        guard let path = resolvePath(cleaned, cwd: cwd) else {
            return pathJailReject("tail", cleaned)
        }
        guard let content = try? String(contentsOfFile: path, encoding: .utf8) else {
    ...
    func cmdTouch(_ args: String, cwd: String) -> CommandResult {
        ...
        guard let path = resolvePath(cleaned, cwd: cwd) else {
            return pathJailReject("touch", cleaned)
        }
